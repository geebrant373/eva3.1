...
public class ShopEdit {
		...
		button.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				if (list_1.getItemCount() == 0) {
					LinAllManager.toMessageBox("상점목록을 추가하여 주십시오.");
					return;
				}
	...
	static private void step2(String[] inv_list, Map<Integer, Object> list) {
		...
		for (String s : inv_list) {
			...
			if (shop.isSellingItem(ii.getItem().getItemId())) {
				L1ShopItem bb = shop.getSellingItem(ii.getItem().getItemId());
				L1ShopItem cc = shop.getBuyItem(ii.getItem().getItemId());
				field[0] = String.valueOf(npc.get_npcId());
				field[1] = String.valueOf(bb.getItemId());
				field[2] = String.valueOf(idx);
				field[3] = String.valueOf(bb.getPrice());
				field[4] = String.valueOf(bb.getPackCount());
				if (cc != null) {
					field[5] = String.valueOf(cc.getPrice());
				} else {
					field[5] = String.valueOf(-1);
				}
				field[6] = String.valueOf(bb.getEnchant());
				field[7] = String.valueOf(s);
				idx++;
			} else {
				L1ShopItem cc = shop.getBuyItem(ii.getItem().getItemId());
				field[0] = String.valueOf(npc.get_npcId());
				field[1] = String.valueOf(ii.getItem().getItemId());
				field[2] = String.valueOf(idx);
				field[3] = String.valueOf(0);
				field[4] = String.valueOf(0);
				if (cc != null) {
					field[5] = String.valueOf(cc.getPrice());
				} else {
					field[5] = String.valueOf(-1);
				}
				field[6] = String.valueOf(0);
				field[7] = String.valueOf(s);
				idx++;
			}
	...
	static private void step3(Table table, boolean db) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("DELETE FROM shop WHERE npc_id=?");
			...
			for (TableItem ti : table.getItems()) {
				pstm2 = con.prepareStatement(
						"INSERT INTO shop SET npc_id=?, item_id=? , order_id=?, selling_price=?, pack_count=?, purchasing_price=?, enchant=?, note=?");
				pstm2.setInt(1, Integer.valueOf(ti.getText(0)));
				pstm2.setInt(2, Integer.valueOf(ti.getText(1)));
				pstm2.setInt(3, Integer.valueOf(ti.getText(2)));
				pstm2.setInt(4, Integer.valueOf(ti.getText(3)));
				pstm2.setInt(5, Integer.valueOf(ti.getText(4)));
				pstm2.setInt(6, Integer.valueOf(ti.getText(5)));
				pstm2.setInt(7, Integer.valueOf(ti.getText(6)));
				pstm2.setString(8, ti.getText(7));
				pstm2.execute();
			}
		} catch (SQLException e) {
