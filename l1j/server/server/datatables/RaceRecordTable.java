...
public class RaceRecordTable {
	...
	private static Logger _log = Logger.getLogger(RaceRecordTable.class.getName());
	...
	public void updateRaceRecord(int number, int win, int lose) {
		...
		try {
			...
			pstm = con.prepareStatement("UPDATE race_record SET win=?,lose=? WHERE number=?");
			pstm.setInt(1, win);
			pstm.setInt(2, lose);
	...
	public void getRaceRecord(int number, L1LittleBugInstance bug) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT win, lose FROM race_record WHERE number=?");
