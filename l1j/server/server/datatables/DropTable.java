...
import l1j.server.random.RndEx;
...
public class DropTable {
	...
	public ArrayList<L1Drop> getDropList(int mobId) {
		ArrayList<L1Drop> dropList = this._droplists.get(Integer.valueOf(mobId));
		if (dropList == null)
			return null;
		return dropList;
	}
	...
	public void setDrop(L1NpcInstance npc, L1Inventory inventory) {
		...
		if (dropList == null) {
			if (DropEventTable.getInstance().isDropEvent(npc.getMapId())) {
				DropEventTable.getInstance().storeEventItem(npc);
			}
			return;
		}
		...
		for (L1Drop drop : dropList) {
			...
			double mapeventadena = Map_Event.getInstance().getadena_rate(npc.getMapId());
			...
			if (itemId == L1ItemId.ADENA) { // 드롭이 아데나의 경우는 아데나레이트를 건다
				if (npc.getMapId() == 410) {
					...
					itemCount *= adenarate * mapeventadena;
				}
			}
			if (itemCount < 0) {
				itemCount = 0;
	...
	public void dropShare(L1NpcInstance npc, ArrayList<?> acquisitorList, ArrayList<?> hateList, L1PcInstance pc) {
		...
		for (int i = hateList.size() - 1; i >= 0; i--) {
			...
			if ((Config.AUTO_LOOT == 0) // 오토 루팅 2의 경우는 사몬 및 애완동물은 생략한다
		...
		for (int i = inventory.getSize(); i > 0; i--) {
			...
			if (Config.AUTO_LOOT != 0 && pc.getMapId() != -1 && totalHate > 0) {
				//System.out.println(Config.AUTO_LOOT);
									...
									if (player.isInParty()) { // 파티의 경우
										...
										if (item != null && item.getItemId() != L1ItemId.ADENA) {
											...
											if (player.getLocation().getTileLineDistance(pc1.getLocation()) < 14) {
												...
												S_SystemMessage pck = new S_SystemMessage(String.format("\\f9[파티]\\fH %s님께서 [%s]을 [%d]개 획득하셨습니다.", 이름, 아이템이름, 아이템갯수));
												...
												for (int k = 0; k < pickup_item.length; k++) {
													if (item.getItemId() == Integer.parseInt(pickup_item[k])) {
														...
														String message = String.format("" + men + "\\fH %s\\f2을(를) 획득하였습니다.", itemName);
											...
											for (int A = 0; A < _membersList.size(); A++) {
												...
												if (!_membersList.get(A).getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_MENT)) {
													_membersList.get(A).sendPackets(new S_SystemMessage(String.format("\\f9[파티]\\fH 아데나 [%d]를 획득하였습니다.", 아데나2)));
												}
											}
										}
									} else {
										...
										if (item != null && item.getItemId() == L1ItemId.ADENA) {
											if (player.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.AdenRateByDoll)) {
												if (player.getAutoHunt()) {
													item.setCount((int) (item.getCount()*2 * Config.자동패널티배율));
												} else {
													item.setCount(item.getCount()*2);
												}
											} else {
												if (player.getAutoHunt()) {
													item.setCount((int) (item.getCount() * Config.자동패널티배율));
												} else {
													item.setCount(item.getCount());
												}
											}
										}
										if (!player.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_MENT)) {
		...
		try {
			if (pc.getParty() == null) {				
				...
				if (EventItemInfo != null) {
					tradeEventSystem_Item(pc, npc, EventItemInfo, lastInventory);
				}
			} else {		
				...
				if (isAllEventOwner) {
					tradeEventSystem_Item(pc, npc, minEventInfo, lastInventory);
				}
	...
	private void tradeEventSystem_Item(L1PcInstance pc, L1NpcInstance npc, EventSystemInfo bonusInfo, L1Inventory inventory) {
		String[] eventbonusitem = (String[])ArrangeParser.parsing(bonusInfo.get_drop_item(), ",", ArrangeParseeFactory.createStringArrange()).result();
		if (!npc.isResurrect() && pc.getMapId() == bonusInfo.get_event_map_id()) {
			for (int i = 0; i < eventbonusitem.length; i++) {
				if (RndEx.isWinning(1000000, bonusInfo.calc_probability(bonusInfo, i, pc)) && bonusInfo.is_event()) {
					L1ItemInstance bonusitem = ItemTable.getInstance().createItem(Integer.parseInt(eventbonusitem[i]));
					bonusitem.setCount(bonusInfo.calc_count(bonusInfo, i, pc));
					/*pc.sendPackets(new S_ServerMessage(143, npc.getName(),
							String.format("%s (%,d)", bonusitem.getName(), bonusitem.getCount())));*/
					inventory.storeTradeItem(bonusitem);
				}
			}
		}
	}
