...
public class ClanTable {
		...
		for (L1Clan clan : L1World.getInstance().getAllClans()) {
			...
			try {
				...
				while (rs.next()) {
					...
					clan.addClanMember(name, rank, 0, null);
				}
			} catch (SQLException e) {
				_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
			} finally {
	...
	public L1Clan createClan(L1PcInstance player, String clan_name) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO clan_data SET clan_id=?, clan_name=?, leader_id=?, leader_name=?, hascastle=?, hashouse=?, emblem_id=?");
			pstm.setInt(1, clan.getClanId());
		...
		clan.addClanMember(player.getName(), player.getClanRank(), player.getOnlineStatus(), player);
		try {
			player.save();
		} catch (Exception e) {
			_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
	...
	public L1Clan createClan(L1PcInstance player, String clan_name, int clanid) {
		...
		try {
			...
			pstm = con.prepareStatement(
					"INSERT INTO clan_data SET clan_id=?, clan_name=?, leader_id=?, leader_name=?, hascastle=?, hashouse=?, emblem_id=?");
			pstm.setInt(1, clan.getClanId());
			pstm.setString(2, clan.getClanName());
			pstm.setInt(3, clan.getLeaderId());
			pstm.setString(4, clan.getLeaderName());
	...
	public void updateClan(L1Clan clan) {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE clan_data SET clan_id=?, leader_id=?, leader_name=?, hascastle=?, hashouse=?, allianceId=?, emblem_id=? WHERE clan_name=?");
			pstm.setInt(1, clan.getClanId());
			...
			pstm.setInt(6, 0);
			pstm.setInt(7, clan.getEmblemId());
			pstm.setString(8, clan.getClanName());
