...
public class BoardSellTable {
  private static Logger _log = Logger.getLogger(BoardSellTable.class.getName());
  
  private static BoardSellTable _instance;
  
  public static BoardSellTable getInstance() {
    if (_instance == null)
      _instance = new BoardSellTable(); 
    return _instance;
  }
  
  private Map<Integer, L1BoardAdena> _boards = new ConcurrentHashMap<>();
  
  private BoardSellTable() {
    Connection con = null;
    PreparedStatement pstm = null;
    ResultSet rs = null;
    try {
      con = L1DatabaseFactory.getInstance().getConnection();
      pstm = con.prepareStatement("SELECT * FROM board_gm_sell");
  ...
  public void writeBoardAdena(L1BoardAdena board) {
    ...
    try {
      con = L1DatabaseFactory.getInstance().getConnection();
      pstm = con.prepareStatement("INSERT INTO board_gm_sell SET id=?, name=?, date=?, title=?, content=?,type=?,trade_number=?,cha_id=? , trade_id=?, trade_name=?, trade_account_name=?, adena_count=?,sell_count=?");
      ...
      pstm.setInt(9, board.getTradeId());
      pstm.setString(10, board.getTradeName());
      pstm.setString(11, board.getTradeAccountName());
      pstm.setInt(12, board.getAdenaCount());
      pstm.setInt(13, board.getSellCount());
      pstm.execute();
      this._boards.put(Integer.valueOf(board.getTradeNumber()), board);
    } catch (SQLException e) {
      _log.log(Level.SEVERE, e.getLocalizedMessage(), e);
  ...
  public void deleteBoardAdena(int number) {
    ...
    try {
      con = L1DatabaseFactory.getInstance().getConnection();
      pstm = con.prepareStatement("DELETE FROM board_gm_sell WHERE trade_number=?");
  ...
  public void updateBoardAdena(L1BoardAdena board) {
    ...
    try {
      con = L1DatabaseFactory.getInstance().getConnection();
      pstm = con.prepareStatement("UPDATE board_gm_sell SET id=?, name=?, content=?, type=?, cha_id=?, trade_id=?, trade_name=?, trade_account_name=? WHERE trade_number=?");
      pstm.setInt(1, board.getId());
      pstm.setString(2, board.getName());
      pstm.setString(3, board.getContent());
      pstm.setInt(4, board.getType());
      pstm.setInt(5, board.getChaId());
      pstm.setInt(6, board.getTradeId());
      pstm.setString(7, board.getTradeName());
      pstm.setString(8, board.getTradeAccountName());
      pstm.setInt(9, board.getTradeNumber());
      pstm.execute();
    } catch (SQLException e) {
      _log.log(Level.SEVERE, e.getLocalizedMessage(), e);
    } finally {
