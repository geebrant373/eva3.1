...
import l1j.server.server.datatables.WeaponMagicNpcBalanceTable.L1WeaponBalance;
import l1j.server.server.utils.SQLUtil;

public class WeaponMagicNpcBalanceTable {
	private static WeaponMagicNpcBalanceTable _instance;
	private Map<Integer, ArrayList<L1WeaponBalance>> _list = new HashMap<Integer, ArrayList<L1WeaponBalance>>();

	public static WeaponMagicNpcBalanceTable getInstance() {
		if (_instance == null) {
			_instance = new WeaponMagicNpcBalanceTable();
		}
		return _instance;
	}

	private WeaponMagicNpcBalanceTable() {
		loadWeaponBalance();
	}

	public static void reload() {
		WeaponMagicNpcBalanceTable oldInstance = _instance;
		_instance = new WeaponMagicNpcBalanceTable();
	...
	private void loadWeaponBalance() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM weapon_magic_damege_npc");
