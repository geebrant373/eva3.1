...
public class AccessoryEnchantInformationTable1 {
	private static AccessoryEnchantInformationTable1 _instance;
	private final Map<Integer, HashMap<Integer, Integer>> _list = new HashMap<Integer, HashMap<Integer, Integer>>();

	public static AccessoryEnchantInformationTable1 getInstance() {
		if(_instance == null) {
			_instance = new AccessoryEnchantInformationTable1();
		}
		return _instance;
	}
	private AccessoryEnchantInformationTable1() {
		loadInformation();
	}
	public static void reload() {
		AccessoryEnchantInformationTable1 oldInstance = _instance;
		_instance = new AccessoryEnchantInformationTable1();
	...
	private void loadInformation() {
		...
		try {
			...
			pstm = con.prepareStatement("SELECT * FROM ¿ÂΩ≈±∏¡÷πÆº≠_¿Œ√¶»Æ∑¸ WHERE item_id = ?");
	...
	private ArrayList<Integer> enumItemIds() {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT DISTINCT item_id FROM ¿ÂΩ≈±∏¡÷πÆº≠_¿Œ√¶»Æ∑¸");
			rs = pstm.executeQuery();
			while (rs.next()) {
