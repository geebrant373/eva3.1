...
public class NpcTable {
	...
	private void loadNpcData() {
		...
		try {
			...
			while (rs.next()) {
				...
				npc.setTransformRate(rs.getInt("transform_rate"));
	...
	private void loadMannequinData() {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			...
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("SELECT * FROM npc_mannequin");
			rs = pstm.executeQuery();
			L1Npc npc  = null;
			while (rs.next()) {
				npc = new L1Npc();
				...
				npc.set_gfxid(rs.getInt("gfxid"));
				npc.set_lawful(rs.getInt("lawful"));
				npc.set_title(rs.getString("title"));
				npc.set_passispeed(rs.getInt("passispeed"));
				npc.set_doppel(true);
	...
	public String findNpcIdByName(int npcid) {
		for (L1Npc npc : _npcs.values()) {
