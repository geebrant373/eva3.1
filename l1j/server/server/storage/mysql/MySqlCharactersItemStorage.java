...
public class MySqlCharactersItemStorage extends CharactersItemStorage {
	...
	public void storeItem(int objId, L1ItemInstance item) throws Exception {
		...
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO character_items SET id = ?, item_id = ?, char_id = ?, item_name = ?, count = ?, is_equipped = 0, enchantlvl = ?, is_id = ?, durability = ?, charge_count = ?, remaining_time = ?, last_used = ?, bless = ?, attr_enchantlvl = ?, end_time = ?, second_id=?, round_id=?, ticket_id=?, limit_time=?, trade_char_id=?, bless_level = ?");
			...
			pstm.setTimestamp(18, item.getLimitTime());
			pstm.setInt(19, item.getTradeCha());
	...
	@Override
	public void updateTradeCha(L1ItemInstance item) throws Exception {
		executeUpdate(item.getId(), "UPDATE character_items SET trade_char_id = ? WHERE id = ?", item.getTradeCha());
		item.getLastStatus().updateTradeCha();
	}
	...
	public void updateItemAll(L1ItemInstance item) throws Exception {
		...
		try {
			...
			pstm.setTimestamp(12, item.getEndTime());
			pstm.setInt(13, item.getTradeCha());
			pstm.setInt(14, item.get_bless_level());
			pstm.setInt(15, item.getId());
