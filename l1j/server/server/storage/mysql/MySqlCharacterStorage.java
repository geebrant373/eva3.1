...
import l1j.server.server.datatables.CharacterAutoHuntTable;
...
public class MySqlCharacterStorage implements CharacterStorage {
	...
	public L1PcInstance loadCharacter(String charName) {
		...
		try {
			...
			pc.setMemberShip(rs.getInt("membership"));
			pc.setVipLevel(rs.getInt("vip_level"));
			pc.setKills(rs.getInt("PC_Kill"));
			pc.setDeaths(rs.getInt("PC_Death"));
			pc.setRankLevel(rs.getInt("rank_level"));
			pc.setHuntPrice(rs.getInt("HuntPrice")); //현상금
			...
			pc.setBirthday(rs.getInt("Birth_day"));
		      String slotTemp = rs.getString("slot1");
		      if (slotTemp != null && slotTemp.length() > 0) {
		        String[] slots = slotTemp.split(",");
		        int[] slot1 = null;
		        slot1 = new int[slots.length];
		        for (int i = 0; i < slots.length; i++)
		          slot1[i] = Integer.parseInt(slots[i]); 
		        pc.setSlot1(slot1);
		      } 
		      String slotTemp2 = rs.getString("slot2");
		      if (slotTemp2 != null && slotTemp2.length() > 0) {
		        String[] slots = slotTemp2.split(",");
		        int[] slot2 = null;
		        slot2 = new int[slots.length];
		        for (int i = 0; i < slots.length; i++)
		          slot2[i] = Integer.parseInt(slots[i]); 
		        pc.setSlot2(slot2);
		      } 
			pc.refresh();
			...
			try {
				CharacterAutoHuntTable.getInstance().load(pc);
			} catch (Exception e) {
				e.printStackTrace();
			}
	...
	public void createCharacter(L1PcInstance pc) {
		...
		try {
			int i = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("INSERT INTO characters SET account_name=?,objid=?,char_name=?,level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,BaseStr=?,Con=?,BaseCon=?,Dex=?,BaseDex=?,Cha=?,BaseCha=?,Intel=?,BaseIntel=?,Wis=?,BaseWis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,로테이션=?,HomeTownID=?,Contribution=?,Pay=?,HellTime=?,Banned=?,Karma=?,LastPk=?,DeleteTime=?,ReturnStat=?,GdungeonTime=?,Ainhasad_Exp=?,Logout_time=?, age=?, membership=?, vip_level=?, PC_Kill=?, PC_Death=?, rank_level=?,HuntPrice=?,wanted=?, AddDamage=?,AddDamageRate=?,AddReduction=?,AddReductionRate=?,Birth_day=?");
			...
			pstm.setInt(++i, pc.getMemberShip());
			pstm.setInt(++i, pc.getVipLevel());
	...
	public void storeCharacter(L1PcInstance pc) {
		...
		try {
			int i = 0;
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con.prepareStatement("UPDATE characters SET level=?,HighLevel=?,Exp=?,MaxHp=?,CurHp=?,MaxMp=?,CurMp=?,Ac=?,Str=?,BaseStr=?,Con=?,BaseCon=?,Dex=?,BaseDex=?,Cha=?,BaseCha=?,Intel=?,BaseIntel=?,Wis=?,BaseWis=?,Status=?,Class=?,Sex=?,Type=?,Heading=?,LocX=?,LocY=?,MapID=?,Food=?,Lawful=?,Title=?,ClanID=?,Clanname=?,ClanRank=?,BonusStatus=?,ElixirStatus=?,ElfAttr=?,PKcount=?,ExpRes=?,PartnerID=?,AccessLevel=?,OnlineStatus=?,HomeTownID=?,Contribution=?,HellTime=?,Banned=?,Karma=?,LastPk=?,DeleteTime=?,ReturnStat=?,GdungeonTime=?,Ainhasad_Exp=?,Logout_time=?, age=?, membership=?, vip_level=?, PC_Kill=?, PC_Death=?, rank_level=?,HuntPrice=?,wanted=?,MaanDelay=?, AddDamage=?,AddDamageRate=?,AddReduction=?,AddReductionRate=?,Birth_day=? WHERE objid=?");
			pstm.setInt(++i, pc.getLevel());
			pstm.setInt(++i, pc.getHighLevel());
			pstm.setInt(++i, pc.getExp());
			pstm.setInt(++i, pc.getBaseMaxHp());
			...
			try {
				CharacterAutoHuntTable.getInstance().store(pc);
			} catch (Exception e) {
				e.printStackTrace();
			}
			_log.finest("stored char data:" + pc.getName());
