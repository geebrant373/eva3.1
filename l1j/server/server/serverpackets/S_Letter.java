...
public class S_Letter extends ServerBasePacket {
	...
	public S_Letter(L1ItemInstance item, boolean trade) {
		buildPacket(item, trade);
	}
	...
	private void buildPacket(L1ItemInstance item, boolean trade) {
		Connection con = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		try {
			con = L1DatabaseFactory.getInstance().getConnection();
			pstm = con
					.prepareStatement("SELECT * FROM auction_letter WHERE item_object_id=?");
			pstm.setInt(1, item.getId());
			rs = pstm.executeQuery();
			while (rs.next()) {
				writeC(Opcodes.S_OPCODE_LETTER);
