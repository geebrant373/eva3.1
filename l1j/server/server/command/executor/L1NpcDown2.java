...
public class L1NpcDown2 implements L1CommandExecutor {
	...
	public void execute(L1PcInstance pc, String cmdName, String poby) {
		try {
			...
			pc.sendPackets(new S_SystemMessage(MapsTable.getInstance().locationname(mapId)+" / "+ mapId+"번 맵"  + " : 삭제: " + removeCount	+ " / 배치: " + spawnCount));
		
		} catch (Exception e) {
		
			pc.sendPackets(new S_SystemMessage("."+cmdName+" <맵 번호> "));
		}
	}
}
