...
public class UserCommands {
	...
	public void BloodParty(L1PcInstance pc) {
		...
		if (pc.getClanRank() == 1 || pc.getClanRank() == 2 || pc.getClanRank() == 3 || pc.isCrown()) { // Clan[O] [군주,수호기사]
			for (L1PcInstance SearchBlood : clan.getOnlineClanMember()) {
				if (SearchBlood.getClanid() != ClanId || SearchBlood.isPrivateShop() || SearchBlood.isAutoClanjoin() || SearchBlood.isInParty() || SearchBlood.getAutoHunt()) {
					continue; // 포문탈출
		...
		case "autohunt-on":
			if (pc.getAutoHunt()) {
				...
				if (pc.isAutoPotion()) {
					...
					pc.setHealItemNum(0);
					pc.setHealVal(0);
					pc.sendPackets(new S_SystemMessage("\\fT자동사냥중에는 별로의 자동물약은 중단됩니다."));
				}
				if (pc.is_자동물약사용()) {
					pc.set_자동물약사용(false);
				}
