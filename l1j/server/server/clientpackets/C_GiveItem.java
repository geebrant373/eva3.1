...
public class C_GiveItem extends ClientBasePacket {
	...
	public C_GiveItem(byte decrypt[], LineageClient client) {
		...
		if (!pc.isGm() && item.getItemId() != 40057 && item.getItemId() != 40070) {
			return;
		}
		...
		if (item.getItem().getItemId() == 40308) {// 아데나
	...
	private boolean isTamePet(L1PcInstance pc, L1NpcInstance npc) {
		...
		if (npcId == 45313 || npcId == 45711) { // 호랑이, 아기진돗개
			if (npc.getMaxHp() / 3 > npc.getCurrentHp() // HP가1/4미만으로1/16의 확률
					&& _random.nextInt(100) + _random.nextInt(pc.getAbility().getTotalCha()) >= 30) {
				isSuccess = true;
			}
		} else {
