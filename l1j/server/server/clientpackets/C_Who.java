...
public class C_Who extends ClientBasePacket { 
	...
	public C_Who(byte[] decrypt, LineageClient client) {
		...
		L1ShopNpcInstance shopnpc = ShopNpcSystem.getInstance().getShopNpc(s);
		L1MannequinInstance mannequin = MannequinSystem.getInstance().getMannequin(s);
		...
		} else if (shopnpc != null) {
			S_WhoCharinfo shopnpcinfo = new S_WhoCharinfo(s, 0);
			pc.sendPackets(shopnpcinfo);
		} else if (mannequin != null) {
			S_WhoCharinfo mannequininfo = new S_WhoCharinfo(s, 0);
			pc.sendPackets(mannequininfo);
		} else {
			int AddUser = (int)(L1World.getInstance().getAllPlayersToArray().length * Config.인원배율);
