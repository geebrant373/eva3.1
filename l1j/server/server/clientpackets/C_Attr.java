...
public class C_Attr extends ClientBasePacket {
	...
	public C_Attr(byte abyte0[], LineageClient clientthread) throws Exception {
					...
					if (clan != null) {
						...
						if (joinPc.getClanid() == 0) { // 크란미가입
							...
							joinPc.sendPackets(new S_ServerMessage(95, clanName)); // \f1%0
																					// 혈맹에
																					// 가입했습니다.
							joinPc.sendPackets(new S_SkillSound(joinPc.getId(), 8952)); // 혈맹버프
		...
		case 223: // %0%s 동맹을 원합니다. 받아들이시겠습니까? (Y/N)
			...
			if (readC() == 1) {
				if (pc.getClan().getAlliance() != null) {
					...
					pc.sendPackets(new S_ServerMessage(224, AllianceClanName, PcClanName));// %0
																							// 혈맹과
																							// %1
																							// 혈맹이
																							// 동맹을
		...
		case 252: // %0%s가 당신과 아이템의 거래를 바라고 있습니다. 거래합니까? (Y/N)
			...
			if (trading_partner != null) {
				if (trading_partner instanceof L1PcInstance) {
					...
					if (c == 0) { // No
						target.sendPackets(new S_ServerMessage(253, pc.getName())); // %0%d는
																					// 당신과의
																					// 거래에
																					// 응하지
																					// 않았습니다.
		...
		case 622: // 설문
			c = readC();
			switch (pc.getAttrMsgType()) {
			case 0:
				if (L1Question.maintext != null && !L1Question.maintext.equals("")) {
					if (c == 0)
						L1Question.bad += 1;
					else if (c == 1)
						L1Question.good += 1;
				...
				break;
			case 1:
				pc.setAttrMsgType(0); // 초기화
				if (c == 0) {
					pc.sendPackets(new S_SystemMessage("상대방의 판매 취소를 동의하지 않았습니다."));
					// 비밀번호 변경이 취소 되었습니다.
				} else if (c == 1) {
					// deleteAdena(pc.getAdenaBuyCount());
					deleteAdenaSell(pc, pc.getAdenaBuyCount());
				}
				break;
			}
			break;
		...
		case 654: // %0%s당신과 결혼 하고 싶어하고 있습니다. %0과 결혼합니까? (Y/N)
			...
			if (partner != null) {
				if (c == 0) { // No
					...
					pc.save();
					pc.sendPackets(new S_ServerMessage(790)); // 모두의 축복 중(안)에서,
																// 두 명의 결혼을
																// 했습니다.
		...
		case 2360:
			c = readC();
			if (c == 1) {
				if (pc.TradeChaItem != null) {
					String ChaName = CharacterTable.getTradeChaName(pc.TradeChaItem.getTradeCha());
					CharacterTable.TradeAccUpdate(pc.getNetConnection().getAccount().getName(), ChaName);
					pc.getInventory().removeItem(pc.TradeChaItem, 1);

					clientthread.CharReStart(true);
					clientthread.sendPacket(new S_PacketBox(S_PacketBox.LOGOUT));

					pc.TradeChaItem = null;
					pc.save();
					pc.saveInventory();
					...
					new C_NoticeClick(clientthread);
				}
			}
			break;
			...
			if (readC() == 1) {
				...
				if (s.toLowerCase().equals("str".toLowerCase())) {
					if (pc.getAbility().getStr() < 35) {
						...
						pc.sendPackets(new S_ServerMessage(481)); // 하나의 능력치의
																	// 최대치는
																	// 25입니다. 다른
																	// 능력치를 선택해
																	// 주세요
				...
				pc.CheckStatus();
				// 추가 리스없이 스텟창 자동 갱신 By.Bentley
				if (pc.getLevel() >= 51 && pc.getLevel() - 50 > pc.getAbility().getBonusAbility()) {
					if ((pc.getAbility().getStr() + pc.getAbility().getDex() + pc.getAbility().getCon() + pc.getAbility().getInt()
							+ pc.getAbility().getWis() + pc.getAbility().getCha()) < 150) {
						String htmlid = null;
						String[] htmldata = null;
						htmlid = "RaiseAttr";
						htmldata = (new String[] { "" + pc.getAbility().getStr() + "", "" + pc.getAbility().getDex() + "",
								"" + pc.getAbility().getCon() + "", "" + pc.getAbility().getWis() + "", "" + pc.getAbility().getInt() + "",
	...
	private void changeClan(LineageClient clientthread, L1PcInstance pc, L1PcInstance joinPc, int maxMember) {
		...
		if (clan != null && oldClan != null && joinPc.isCrown() && joinPc.getId() == oldClan.getLeaderId()) {
			...
			for (int cnt = 0; cnt < clanMember.length; cnt++) {
				clanMember[cnt].sendPackets(new S_ServerMessage(94, joinPc.getName())); // \f1%0이
																						// 혈맹의
																						// 일원으로서
																						// 받아들여졌습니다.
			}
			...
			for (int i = 0; i < oldClan.getClanMemberList().size(); i++) {
				...
				if (oldClanMember != null) { // 온라인중의 구크란 멤버
					...
					try {
						...
						offClanMember.save();
						clan.addClanMember(offClanMember.getName(), offClanMember.getClanRank(), offClanMember.getOnlineStatus(), offClanMember);
					} catch (Exception e) {
						_log.log(Level.SEVERE, e.getLocalizedMessage(), e);
					}
				}
	...
	private void deleteAdenaSell(L1PcInstance pc, int id) {
		try {
			id = pc.getAdenaBuyCount();
			CancelAdenaSell(pc, id);
		} catch (Exception e) {
			pc.sendPackets(new S_SystemMessage("구매 취소: 구매 취소가 실패 하였습니다 "));
		}
	}
	...
	private void CancelAdenaSell(L1PcInstance pc, int id) {
		...
		try {
			...
			if (title.equalsIgnoreCase("거래완료")) {
				pc.sendPackets(new S_SystemMessage("판매 취소: 이미 거래 완료된 물품 입니다. "));
				return;
			} else if (title.equalsIgnoreCase("판매중")) {
				pc.sendPackets(new S_SystemMessage("판매 취소: 판매중인 물품은 취소 할 수 없습니다. "));
				return;
			} else if (title.equalsIgnoreCase("판매취소")) {
				pc.sendPackets(new S_SystemMessage("판매 취소: 이미 취소된 물품 입니다. "));
				return;
			} else {
				if (title.equalsIgnoreCase("거래중")) {
					L1PcInstance target = L1World.getInstance().getPlayer(SellerName);
					if (target == null) {
						pc.sendPackets(new S_SystemMessage("판매자가 접속중이지 않습니다. "));
						return;
					} else {
						deleteAdena(target.getAdenaSellCount());
						target.getInventory().storeItem(40308, adena);
						target.setAdenaSellCount(0);
						target.sendPackets(new S_SystemMessage("판매 취소: 등록하신 물품이 취소 되었습니다."));
					}
					pstm2 = con.prepareStatement("UPDATE board_adena SET bidder=?, title=?, step=? WHERE id=?");
					pstm2.setString(1, target.getName());
					pstm2.setString(2, "판매취소");
					pstm2.setInt(3, 3);
					pstm2.setInt(4, pc.getAdenaBuyCount());
					pc.setAdenaBuyCount(0);
					pc.sendPackets(new S_SystemMessage("구매 취소: 등록하신 물품이 취소 되었습니다."));
					pstm2.executeUpdate();
					pstm2.close();
				}
			}
