...
public class C_ReturnStaus extends ClientBasePacket {
	public C_ReturnStaus(byte[] decrypt, LineageClient client) {
        ...
        if(type == 1) {
			...
			L1Teleport.teleport(pc, 32620, 32725, (short)4, 5, true);	
			Restar_World(pc);
	...
	private static void Restar_World(final L1PcInstance pc) {
		pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE,"\\f3피뻥 방지를 위해 잠시 후 종료 됩니다.(스텟,피뻥 임시)"));
		pc.sendPackets(new S_SystemMessage("\\f3피뻥 방지를 위해 잠시 후 종료 됩니다.(스텟,피뻥 임시)"));
