...
public class C_Attack extends ClientBasePacket {
	...
	public C_Attack(byte[] decrypt, LineageClient client) {
		...
		if (target instanceof L1PcInstance) {
			...
			if (pc.get_DuelLine() != 0 && pc.get_DuelLine() == pc2.get_DuelLine()) {
				return;
			}
		}
		 ...
		 * eva.LogBugAppend("스핵:AW-"+(pc.getCurrentWeapon()+1), pc, 1); L1SkillUse
		 ...
		 * S_SystemMessage("허용된 값을 초과하여 행동이 제한됩니다.")); pc.addSpeedHackCount(1); return;
		 * } }
		 * 
		 */
		...
		if (Config.ASBUGCHECK_ALLBUF || pc.getCurrentSpriteId() != Config.ALLBUFF_POLYID) {
			...
			if (pc.getAttack_Time() > currentTime) {
				pc.setAttack_Time_count(pc.getAttack_Time_count() + 1);
		...
		if (Config.CHECK_ATTACK_INTERVAL) {
			...
			case AcceleratorChecker.R_DISCONNECTED:
				if (pc.isGm()) {
					System.out.println("운영자 스핵 검출:" + pc.getName());
				}
		...
		if (target != null && !((L1Character) target).isDead()) {
			...
			if (client.getActiveChar().autoattack) {
				if (target instanceof L1MonsterInstance || target instanceof L1PcInstance
						|| target instanceof L1PetInstance || target instanceof L1SummonInstance
						|| target instanceof L1TowerInstance || target instanceof L1ScarecrowInstance) {
