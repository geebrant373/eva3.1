...
public class C_War extends ClientBasePacket {
	...
	public C_War(byte abyte0[], LineageClient clientthread) throws Exception {
		...
		if (enemyClan.getCastleId() != 0 && // 상대 크란이 성주로, 자캐릭터가 Lv25 미만
			...
			player.sendPackets(new S_SystemMessage("공성전을 선언하려면  레벨 45에 이르지 않으면 안됩니다.(필수 변신해제 후 선포)")); // 공성전을 선언하려면  레벨 25에 이르지 않으면 안됩니다.
			return;
		}

		if (clan.getHouseId() > 0){
			player.sendPackets(new S_SystemMessage("아지트가 있는 상태에서는 선전 포고를 할 수 없습니다.(필수 변신해제 후 선포)"));
			return;
		}
