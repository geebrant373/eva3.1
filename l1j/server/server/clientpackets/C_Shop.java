...
//		}
		...
		for (L1PcInstance target : L1World.getInstance().getAllPlayers()) {
			if (target.getId() != pc.getId() && target.getAccountName().toLowerCase().equals(pc.getAccountName().toLowerCase())
		...
		if (ShopCount >= 2) {
			pc.sendPackets(new S_SystemMessage("무인상점은 최대 2개의 캐릭터만 가능합니다."));
			return;
		}
