...
public class AcceleratorChecker {
	...
	private static final double PEARL_RATE = 0.87;
	...
	public int checkInterval(ACT_TYPE type) {
		...
		//System.out.println("실제시간(Interval):" + interval + ",  spr값 :" + rightInterval);
...
*/
	...
	public int getRightInterval(ACT_TYPE type) {
		...
		case ATTACK:
			...
			if(interval == 0){
				System.out.println("없는(공격) gfxid:"+ _pc.getGfxId().getTempCharGfx());
				interval = 700;//700
			}
		...
		case MOVE:
			...
			if(interval == 0){
				System.out.println("없는(이동) gfxid:"+ _pc.getGfxId().getTempCharGfx());
			}
			break;
		case SPELL_DIR:
