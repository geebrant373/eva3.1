...
public class L1ParalysisPoison extends L1Poison {
		...
		public void run() {
			...
			if (_target instanceof L1PcInstance) {
				...
				if (player.isDead() == false) {
					...
					((L1PcInstance) _target).sendPackets(new S_SkillIconAura(227, 8));
					if (isInterrupted()) {
						_timer.interrupt();
					}
				}
	...
	private void doInfection() {
		...
		if (_target instanceof L1PcInstance) {
			...
			GeneralThreadPool.getInstance().execute(_timer);
			((L1PcInstance) _target).sendPackets(new S_SkillIconAura(227, 16));
		}
	}
	...
	public void cure() {
		...
		((L1PcInstance) _target).sendPackets(new S_SystemMessage("몸에서 독기운이 빠져나갑니다."));
		((L1PcInstance) _target).sendPackets(new S_SkillIconAura(227, 0.1));
