...
public class WeaponSkill {
	...
	public static void getDiseaseWeapon(L1PcInstance pc, L1Character cha) {
	    ...
	    switch (enchantLevel) {
	        case 9: diseaseChance = Config.무기디지즈확률1; break;
	        case 10: diseaseChance = Config.무기디지즈확률2; break;
	        case 11: diseaseChance = Config.무기디지즈확률3; break;
	        case 12: diseaseChance = Config.무기디지즈확률4; break;
	        case 13: diseaseChance = Config.무기디지즈확률5; break;
	        case 14: diseaseChance = Config.무기디지즈확률6; break;
	        default: return;
	    }
	    ...
	    if (cha instanceof L1PcInstance) {
	    	...
	    	if (chance <= diseaseChance) {
				if (!target.getSkillEffectTimerSet().hasSkillEffect(56)) {
					target.addDmgup(-6);
					target.getAC().addAc(12);
					target.sendPackets(new S_OwnCharAttrDef(target));
					target.getSkillEffectTimerSet().setSkillEffect(56, skilltime * 1000);
					S_SkillSound sound = new S_SkillSound(target.getId(), 2230);
					pc.sendPackets(sound);
					Broadcaster.broadcastPacket(pc, sound);
				}
			}
	    } else if (cha instanceof L1MonsterInstance) {
	    	L1MonsterInstance targetMob = (L1MonsterInstance) cha;
	    	if (chance <= diseaseChance) {
				if (!targetMob.getSkillEffectTimerSet().hasSkillEffect(56)) {
					targetMob.addDmgup(-6);
					targetMob.getAC().addAc(12);
					targetMob.getSkillEffectTimerSet().setSkillEffect(56, skilltime * 1000);
					S_SkillSound sound = new S_SkillSound(targetMob.getId(), 2230);
					pc.sendPackets(sound);
					Broadcaster.broadcastPacket(pc, sound);
				}
			}
	...
	public static int getVenomBladesDamage(L1PcInstance pc, L1Character target, int enchant){
		int dmg = 0;
		int chance = _random.nextInt(100)+1;
		if (enchant >= chance){
			dmg = 20;
			if(dmg < 0) dmg = 0;
			pc.sendPackets(new S_SkillSound(target.getId(), 9359));
			Broadcaster.broadcastPacket(pc, new S_SkillSound(target.getId(), 9359));			
		}
		return dmg;
	}
	...
	public static double getKurtSwordDamage(L1PcInstance pc, L1Character cha) {
		...
		if (7 >= chance) {
			...
			dmg = 150 + magic.calcMagicDamage(L1SkillId.CALL_LIGHTNING);
	...
	public static double get악몽의장궁(L1PcInstance pc, L1Character cha, int enchant) {
		double dmg = 0;
		int chance = _random.nextInt(100) + 1;
		if (15 >= chance) {
			dmg = 10 + _random.nextInt(enchant);
			S_UseAttackSkill packet = new S_UseAttackSkill(pc, cha.getId(),
					10, cha.getX(), cha.getY(), ActionCodes.ACTION_Attack, false);
	...
	public static double getNightmareBowDamage(L1PcInstance pc, L1Character cha, int enchant) {
		double dmg = 0;
		int locx = cha.getX();
		int locy = cha.getY();
		...
		int chance = _random.nextInt(100) + 1;
		if (4 + enchant >= chance) {
			int div = (int)((dex / 10) * 1.1);
			int min = 20 + div;
			int max = 50 + div;	
			dmg = _random.nextInt(max - min) + min;
			if(dmg <= 0) dmg = 0;

			S_EffectLocation packet = new S_EffectLocation(locx, locy, 13541);
			pc.sendPackets(packet);
	...
	public static double getMoonBowDamage(L1PcInstance pc, L1Character cha, int enchant) {
		...
		int chance = _random.nextInt(100) + 1;
		if (15 >= chance) {
			dmg = 10 + _random.nextInt(enchant+1);
			S_UseAttackSkill packet = new S_UseAttackSkill(pc, cha.getId(),
	...
	public static double get4차StaffDamage(L1PcInstance pc, L1Character cha) {
		double dmg = 0;
		int chance = _random.nextInt(100) + 1;
		if (15 >= chance) {
			int locx = cha.getX();
			...
			S_EffectLocation packet = new S_EffectLocation(locx, locy, 1811);
			pc.sendPackets(packet);
			Broadcaster.broadcastPacket(pc, packet);
			magic = null;
		}
