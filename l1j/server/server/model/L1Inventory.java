...
public class L1Inventory extends L1Object {
	...
	public int checkAddItem(L1ItemInstance item, int count) {
		...
		if (getSize() > Config.MAX_NPC_ITEM || (getSize() == Config.MAX_NPC_ITEM && (!item.isStackable() || !checkItem(item.getItem().getItemId())))
				|| (getSize() == Config.MAX_NPC_ITEM && item.getItem().getItemId() == 40309)) { // 용량
																								// 확인
			return SIZE_OVER;
		}
	...
	public synchronized L1ItemInstance storeItem(int id, int count, int enchant, int bless) {
		if (count <= 0) {
			return null;
		}
	...
	public synchronized L1ItemInstance storeItem(int id, int count, int enchant) {
		...
		if (id == 40308) {
			if (count > 5000000) {
				System.out.println("[22222] 1천만 이상 아데나가 갑자기 생성됨");
			}
		}
		L1Item temp = ItemTable.getInstance().getTemplate(id);
		if (temp == null) {
	...
	public L1ItemInstance findItemId(int id, int bless) {
		return findItemId(id, bless, 1);
	}

	public synchronized L1ItemInstance storeItem(int id, int count, boolean identified) {
		if (count <= 0) {
			return null;
		}
		L1Item temp = ItemTable.getInstance().getTemplate(id);
	...
	public L1ItemInstance findItemId(int id, int bless, int count) {
		L1ItemInstance findItem = null;
		for (L1ItemInstance item : this._items) {
			if ((item.getItem().getItemId() == id) && ((bless == -1) || (item.getBless() == bless)) && (item.getCount() >= count)) {
				if (findItem != null) {
	...
	public synchronized L1ItemInstance storeItem(int id, int count) {
		if (id == 40308) {
			if (count > 5000000) {
				System.out.println("1천만 이상 아데나가 갑자기 생성됨");
			}
		}
	...
	public synchronized L1ItemInstance storeItem(L1ItemInstance item) {
		...
		if (item.isStackable() && item.getItem().getItemId() != 40309) {
			L1ItemInstance findItem = findItemId(itemId, item.getBless());
			if (findItem != null) {
				long findCount = findItem.getCount();
				long itemCount = item.getCount();
				long check_count = findCount + itemCount;
				if (findItem.getItemId() == L1ItemId.ADENA && check_count >= 1900000000) {
					if (check_count >= 1900000000) {
			...
			L1ItemInstance findItem = findItemTicketId(40309, item.getSecondId(), item.getTicketId());
			if (findItem != null) {
				findItem.setCount(findItem.getCount() + item.getCount());
				updateItem(findItem);
				return findItem;
		...
		if (item.getItem().getLimitTime() > 0) {
			Timestamp deleteTime = null;
			deleteTime = new Timestamp(System.currentTimeMillis() + (item.getItem().getLimitTime() * 60000));
			item.setLimitTime(deleteTime);
		}
	...
	public synchronized L1ItemInstance storeTradeItem(L1ItemInstance item) {
		if (item.isStackable() && item.getItem().getItemId() != 40309) {
			L1ItemInstance findItem = findItemId(item.getItem().getItemId(), item.getBless());
			if (findItem != null) {
				long findCount = findItem.getCount();
				long itemCount = item.getCount();
				long check_count = findCount + itemCount;
	...
	public boolean consumeItem(int itemid, int count, int enchant) {
	    int remaining = count;
	    for (L1ItemInstance item : getItems()) {
	        if (item.getItemId() == itemid && item.getEnchantLevel() == enchant) {
	            int deleteCount = Math.min(item.getCount(), remaining);
	            removeItem(item, deleteCount, enchant);
	...
	public int removeItem(L1ItemInstance item, int count, int enchant) {
		if (item == null) {
			return 0;
		}
		if (item.getCount() <= 0 || count <= 0) {
		...
		if (item.getEnchantLevel() != enchant) {
			//System.out.println("item.getEnchantLevel() = "+ item.getEnchantLevel());
	...
	public int countItems(int id, int enchant) {
		if (ItemTable.getInstance().getTemplate(id).isStackable()) {
			// 스택형 아이템은 인첸트 개념이 없으니까 기존 countItems 로 처리
	...
	public synchronized L1ItemInstance tradeItem(L1PcInstance pc, int objectId, int count, L1Inventory inventory) {
		L1ItemInstance item = getItem(objectId);
		return tradeItem(pc, item, count, inventory);
	}
	...
	public synchronized L1ItemInstance tradeItem(L1ItemInstance item, int count, L1Inventory inventory) {
		...
		if (item.getCount() <= count || count < 0) {
			...
			carryItem.setAttrEnchantLevel(item.getAttrEnchantLevel());
			carryItem.setLimitTime(item.getLimitTime());
			carryItem.setTradeCha(item.getTradeCha()); // 캐릭교환템
		}
		return inventory.storeTradeItem(carryItem);
	}
	...
	public L1ItemInstance findItemId(String name) {
		for (L1ItemInstance item : _items) {
	...
	public boolean checkItem(int id, int count) {
		...
		L1Item check = ItemTable.getInstance().getTemplate(id);

		if (check != null && check.isStackable()) {
			L1ItemInstance item = findItemId(id);
			if (item != null && item.getCount() >= count) {
				return true;
			}
	...
	public L1ItemInstance findItemTicketId(int id, int secid, int ticketid) {
		for (L1ItemInstance item : _items) {
			if (item.getItem().getItemId() == id && item.getSecondId() == secid && item.getTicketId() == ticketid) {
				return item;
			}
		}
		return null;
	...
	public L1ItemInstance findItemIdBless(int id, int bless) {
		for (L1ItemInstance item : _items) {
			if (item.getItem().getItemId() == id && item.getBless() == bless) {
				return item;
			}
		}
		return null;
	...
	public L1ItemInstance[] findItemsIdNotEquippedCraftBless(int id, int enchant, int bless) {
		ArrayList<L1ItemInstance> itemList = new ArrayList<L1ItemInstance>();
		for (L1ItemInstance item : _items) {
			if (item == null)
				continue;
			if (item.getItemId() == id) {
				if (!item.isEquipped() && item.getEnchantLevel() == enchant && item.getBless() == bless) {
					itemList.add(item);
				}
			}
		}
	...
	public synchronized L1ItemInstance tradeItem(L1PcInstance pc, L1ItemInstance item, int count, L1Inventory inventory) {
		String itemname;
        long itemcount;
        long oldCount;
		...
		*/
		...
		if (pc.getAutoHunt()) {
	        ...
	        if (L1PcInventory.autoHuntItems.containsKey(itemname)) {
	            oldCount = L1PcInventory.autoHuntItems.get(itemname);
	            L1PcInventory.autoHuntItems.put(itemname, oldCount + itemcount);
	        } else {
	        	L1PcInventory.autoHuntItems.put(itemname, itemcount);
	        }
	    }
		return inventory.storeTradeItem(carryItem);
	}
