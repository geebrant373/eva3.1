...
public class L1PcInventory extends L1Inventory {
	...
	private long timeVisibleDelay = 1000;

	public static Map<String, Long> autoHuntItems = new HashMap<>();
...
//		}
	...
	public static final int COL_SAVE_ALL = 8192;
	public static final int COL_TRADE_CHA = 2048;
	
	public static final int COL_ATTRENCHANTLVL = 1024;

	public static final int COL_BLESS = 512;

	public static final int COL_BLESS_LEVEL = 4096;
	...
	public void updateItem(L1ItemInstance item, int column) {
		...
		if (column >= COL_TRADE_CHA) {
			_owner.sendPackets(new S_ItemName(item));
			column -= COL_TRADE_CHA;
		}
...
//				_owner.sendPackets(new S_ItemStatus(item));
	...
	public int mpRegenPerTick() {
		...
		for (Object itemObject : _items) {
			item = (L1ItemInstance) itemObject;
			if (item.isEquipped()) {
				if (item.getItemId() == 329) { // 명지처리
					mpr += item.getEnchantLevel();
				}
	...
	public L1ItemInstance CaoPenalty() {
	    List<L1ItemInstance> candidateItems = new ArrayList<>();
	    ...
	    for (L1ItemInstance item : _items) {
	        ...
	        if (!item.getItem().isTradable()) {
	            continue; // 거래 불가능한 아이템 제외
	        }

	        boolean isPetItem = false;
	        for (Object petObject : _owner.getPetList().values()) {
	    ...
	    // 필터링된 아이템 중에서 하나 랜덤 선택
	    Random random = new Random(System.nanoTime());
	    if (candidateItems.size() > 0) {
	    	L1ItemInstance penaltyItem = candidateItems.get(random.nextInt(candidateItems.size()));
	...
	public void checkEndTime() {
		...
		for (Object itemObject : li) {
			item = (L1ItemInstance) itemObject;
			if (item.getLimitTime() != null) {
				if (item.getLimitTime().getTime() < System.currentTimeMillis()) {
					if (item.isEquipped()) {
