...
public class L1Teleport {
	...
	public static void teleport(L1PcInstance pc, int x, int y, short mapId, int head, boolean effectable, int skillType) {
		...
		// 이부분 추가 영자 뚫어 안되면 삭제 - 투명상태일때만 텔레포트안보임220303 
		if (!pc.isGm()) {
	...
	private static void doTeleport(L1PcInstance pc, int x, int y, short mapId, int head) {
		...
		if (oldmap != newmap) {
			int setTimer1 = (Config.기감시간 * 60) - pc.getAccount().getGiranPrisonTime();
			int setTimer2 = (Config.몽섬시간 * 60) - pc.getAccount().getDreamIslandTime();
			int setTimer3 = (Config.라던시간 * 60) - pc.getAccount().getLastabardTime();
			...
			switch (newmap) {
			case 53:
				pc.sendPackets(new S_PacketBox(S_PacketBox.ICON_I2H, setTimer1));
				break;
			case 303:
				pc.sendPackets(new S_PacketBox(S_PacketBox.ICON_I2H, setTimer2));
				break;
			case 530:
			case 531:
			case 532:
			case 533:
				pc.sendPackets(new S_PacketBox(S_PacketBox.ICON_I2H, setTimer3));
				break;
			default:
				break;
			}
