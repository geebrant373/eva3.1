	...
	public static void stopSkill(L1Character cha, int skillId) {
		...
		case 자동사냥시간:
			if (cha instanceof L1PcInstance) {
				...
				pc.sendPackets(new S_SystemMessage("\\fT자동사냥 시간이 소진되었습니다."),true);
				pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\fT자동사냥 시간이 소진되었습니다."));
				L1Teleport.teleport(pc, 33445, 32796, (short) 4, pc.getMoveState().getHeading(), true);
			}
			break;
		case 지배이반버프:
			if (cha instanceof L1PcInstance) {
		...
		case FOG_OF_SLEEPING:
			...
			if (cha instanceof L1PcInstance) {
				...
				pc.sendPackets(new S_OwnCharStatus(pc));
				L1CurseParalysis.sleeping(pc, 0, 0, false);
			}
			break;
		case SHAPE_CHANGE:
		...
		case ADVANCE_SPIRIT:
			...
			break;
		case SHOCK_STUN:
		case EMPIER:  //겜블추가 엠파이어디버프타이머 220326
		...
		case SHADOW_FANG:
			...
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.sendPackets(new S_SkillSound(pc.getId(), 8956)); // 종료
			}
			break;
		case DRESS_MIGHTY:
		...
		case FEATHER_BUFF_C:
			...
			break;
		case STATUS_DRAGONMAAN_EARTH:	//지룡 마안
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.MaanDodge = false;
			...
			break;
		case STATUS_DRAGONMAAN_WATER:	//수룡 마안
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.MaanMagicIm = false;
				pc.getResistance().addFreeze(-3);
			}
			break;
		case STATUS_DRAGONMAAN_WIND:	//풍룡 마안
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.MaanMagicDmg = false;
				pc.getResistance().addSleep(-3);
			}
			break;
		case STATUS_DRAGONMAAN_FIRE:	//화룡 마안
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.MaanAddDmg = false;
			...
			break;
		case STATUS_DRAGONMAAN_BIRTH:	//탄생 마안
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.MaanDodge = false;
				pc.MaanMagicIm = false;
			...
			break;
		case STATUS_DRAGONMAAN_SHAPE:	//형상 마안
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.MaanDodge = false;
				pc.MaanMagicIm = false;
			...
			break;
		case STATUS_DRAGONMAAN_LIFE:	//형상 마안
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
				pc.MaanDodge = false;
				pc.MaanMagicIm = false;
...
private static void CancelAdenaSell(L1PcInstance pc, int id) {
...
//			}
		...
		pc.sendPackets(new S_SystemMessage("판매 취소: 제한 시간이 초과되어 게시물이 삭제됩니다."));
...
class L1SkillTimerTimerImpl implements L1SkillTimer, Runnable {
	...
	@Override
	public void run() {
		if (_skillId == L1SkillId.자동사냥시간) {
			if (_cha.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.자동사냥시간)) {
				if (((L1PcInstance) _cha).getAutoHunt()) {
					_remainingTime--;
				}
			}
		} else {
