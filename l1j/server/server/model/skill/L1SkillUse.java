...
public class L1SkillUse {
	...
	private static final int[] CAST_WITH_INVIS = { 1, 2, 3, 5, 8, 9, 12, 13, 14, 19, 21, 26, 31, 32, 35, 37, 42, 43, 44,
			...
			163, 164, 165, 166, 168, 169, 170, 171, 181, SOUL_OF_FLAME, ADDITIONAL_FIRE };
	...
	private static final int[] EXCEPT_COUNTER_MAGIC = { 1, 2, 3, 5, 8, 9, 12, 13, 14, 19, 21, 26, 31, 32, 35, 37, 42,
			...
			156, 158, 159, 161, 163, 164, 165, 166, 168, 169, 170, 171, 181, SOUL_OF_FLAME, ADDITIONAL_FIRE,
	...
	public boolean checkUseSkill(L1PcInstance player, int skillid, int target_id, int x, int y, String message,
		...
		if (_skillId == MASS_TELEPORT && player.isInvisble()) {
			player.sendPackets(new S_SystemMessage("투명 상태에선 매스 텔레포트를 사용할 수 없습니다."));
			return false;
		}
		...
		if (l1object instanceof L1ShopNpcInstance || l1object instanceof L1MannequinInstance) {
			if (!(_skill.getType() == L1Skills.TYPE_HEAL || _skill.getType() == L1Skills.TYPE_CHANGE)) {
	...
	private boolean isNormalSkillUsable() {
		if (_user instanceof L1PcInstance) {
			...
			if (pc.getInventory().getWeight240() >= 200) { // 중량 오버이면 스킬을 사용할 수
															// 없다
			...
			long nowtime = System.currentTimeMillis();
			if (_skillId == 17 && pc.getSkilldelayCheck() >= nowtime
					|| _skillId == 25 && pc.getSkilldelayCheck() >= nowtime) {
				return false;
			} else if (pc.getSkilldelayCheck() >= nowtime) {
				return false;
			}
			// 추가 /** 버그관련수정 (아이템 마법 노딜수정) By 도우너 **/

			if (_skillId == SHADOW_FANG) {
				if (pc.getWeapon() == null) {
					pc.sendPackets(new S_SystemMessage("무기를 착용해야 사용할 수 있습니다."));
					return false;
				}
			}
			...
			if (_skillId == EMPIER) {
				if (pc.getInventory().getTypeEquipped(2, 7) == 0) {
					pc.sendPackets(new S_SystemMessage("방패 착용시 사용가능한 스킬입니다."));
					return false;
				}
			}
	...
	private boolean isTarget(L1Character cha) throws Exception {
		...
		if (_skillId == IMMUNE_TO_HARM) { // 이뮨처리
			if (CharPosUtil.glanceCheck(cha, _user.getX(), _user.getY()) == false) {
		...
		if (CharPosUtil.glanceCheck(_user, cha.getX(), cha.getY()) == false && _skill.isThrough() == false) {
		...
		if ((cha.getSkillEffectTimerSet().hasSkillEffect(ICE_LANCE)
				...
				&& (_skillId == ICE_LANCE || _skillId == FREEZING_BLIZZARD
						|| _skillId == SHOCK_STUN || _skillId == EMPIER)) {
			return false;
		}
		...
		if (cha instanceof L1PcInstance) {
			...
			if (pc.getSkillEffectTimerSet().hasSkillEffect(ABSOLUTE_BARRIER)) {
				if (_skillId == CURSE_BLIND || _skillId == WEAPON_BREAK || _skillId == DARKNESS || _skillId == WEAKNESS
						...
						|| _skillId == REMOVE_CURSE || _skillId == MOB_SLOW_1
	...
	private void makeTargetList() {
		try {
			...
			if (_skill.getArea() == 0) {
				...
				if (_skill.getSkillId() == L1SkillId.METEOR_STRIKE) {
					if (_target instanceof L1PcInstance) {
						for (L1Object tgobj : objects) {
							if (tgobj == null) {
								continue;
							}
							if (_user.getId() != tgobj.getId()) {
								continue;
							}
	...
	private boolean isHPMPConsume() {
		...
		if (_user instanceof L1NpcInstance) {
			...
			if ((_skillId == HEAL || _skillId == EXTRA_HEAL) && _player.getInventory().checkEquipped(20014)) {
				_mpConsume /= 2;
			}
	...
	private void addMagicList(L1Character cha, boolean repetition) {
		...
		if (_skillId == SHOCK_STUN || _skillId == EMPIER) {
		...
		if ((_skillId == ICE_LANCE || _skillId == FREEZING_BLIZZARD) && !_isFreeze) {
			return;
		}
	...
	private void sendGrfx(boolean isSkillAction) {
		...
		if (_user instanceof L1PcInstance) {
			...
			if (_skillId == SHOCK_STUN || _skillId == MOB_SHOCKSTUN_30 || _skillId == MOB_RANGESTUN_19
					|| _skillId == MOB_RANGESTUN_18 || _skillId == EMPIER) {
			...
			if (_skillId == UNCANNY_DODGE) {
				...
				if (pc.getAC().getAc() <= -100) {
					...
					pc.sendPackets(new S_SkillSound(pc.getId(), 2950, 19));
					pc.broadcastPacket(new S_SkillSound(pc.getId(), 2950, 19));
				}
			}
			...
			if (_skill.getTarget().equals("attack") && _skillId != 18) {
				...
				if (_skill.getArea() == 0) { // PC 일반 공격마법
					if (_target.getGfxId().getTempCharGfx() == 16074 || _target.getGfxId().getTempCharGfx() == 16053
						...
						_player.sendPackets(
								new S_UseAttackSkill(_player, targetid, castgfx, _targetX, _targetY, actionId, 0));
						Broadcaster.broadcastPacket(_player,
								new S_UseAttackSkill(_player, targetid, castgfx, _targetX, _targetY, actionId, 0));
					} else {
						_player.sendPackets(
						...
						Broadcaster.broadcastPacketExceptTargetSight(_target,
								new S_DoActionGFX(targetid, ActionCodes.ACTION_Damage), _player);
					}
				} else {
					...
					if (_target.getGfxId().getTempCharGfx() == 16074 || _target.getGfxId().getTempCharGfx() == 16053
						...
						Broadcaster.broadcastPacket(_player,
								new S_RangeSkill(_player, cha, castgfx, actionId, S_RangeSkill.TYPE_DIR));
					}
				}
				...
				Broadcaster.broadcastPacket(_player,
						new S_RangeSkill(_player, cha, castgfx, actionId, S_RangeSkill.TYPE_NODIR));
			} else {
			...
			if (_skill.getTarget().equals("attack") && _skillId != 18) {
				if (_skill.getArea() == 0) {
					...
					if (_target.getGfxId().getTempCharGfx() == 16074 || _target.getGfxId().getTempCharGfx() == 16053
						...
						Broadcaster.broadcastPacket(_user,
								new S_UseAttackSkill(_user, targetid, castgfx, _targetX, _targetY, actionId, 0));
					} else {
						Broadcaster.broadcastPacket(_user,
								new S_UseAttackSkill(_user, targetid, castgfx, _targetX, _targetY, actionId));
						Broadcaster.broadcastPacketExceptTargetSight(_target,
								new S_DoActionGFX(targetid, ActionCodes.ACTION_Damage), _user);
					}
				} else {
					L1Character[] cha = new L1Character[_targetList.size()];
					int i = 0;
					...
					if (_target.getGfxId().getTempCharGfx() == 16074 || _target.getGfxId().getTempCharGfx() == 16053
						...
						Broadcaster.broadcastPacket(_user,
								new S_RangeSkill(_user, cha, castgfx, actionId, S_RangeSkill.TYPE_DIR));
					}
				}
			} else if (_skill.getTarget().equals("none") && _skill.getType() == L1Skills.TYPE_ATTACK) {
				L1Character[] cha = new L1Character[_targetList.size()];
				...
				for (TargetStatus ts : _targetList) { // 무방향 전체공격마법 NPC
					if (ts.getTarget().getGfxId().getTempCharGfx() == 16074
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16053
							|| ts.getTarget().getGfxId().getTempCharGfx() == 14491
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16056
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16284
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16002
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16040
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16027
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16008
							|| ts.getTarget().getGfxId().getTempCharGfx() == 15986
							|| ts.getTarget().getGfxId().getTempCharGfx() == 16014) {
						Broadcaster.broadcastPacket(_user,
								new S_UseAttackSkill(_user, targetid, castgfx, _targetX, _targetY, actionId, 0));
						Broadcaster.broadcastPacket(_user,
								new S_UseAttackSkill(_user, targetid, castgfx, _targetX, _targetY, actionId, false));
					} else {
						if (ts.getTarget().getGfxId().getTempCharGfx() != 16074
								|| ts.getTarget().getGfxId().getTempCharGfx() != 16053
								|| ts.getTarget().getGfxId().getTempCharGfx() != 14491
								...
								|| ts.getTarget().getGfxId().getTempCharGfx() != 15986
								|| ts.getTarget().getGfxId().getTempCharGfx() != 16014) {
							Broadcaster.broadcastPacket(_user, new S_UseAttackSkill(_user, targetid, castgfx, _targetX,
									_targetY, actionId, false));
						}
					}
				}
			} else {
	...
	private void setDelay() {
		...
		// 추가 /** 버그관련수정 (아이템 마법 노딜수정) By 도우너 **/
		long nowtime = System.currentTimeMillis();
		if (_user.getSkilldelayCheck() <= nowtime) {
			_user.setSkilldelayCheck(nowtime + _skill.getReuseDelay());
		} else {
			_user.setSkilldelayCheck(nowtime + _skill.getReuseDelay());
		}
		// 추가 /** 버그관련수정 (아이템 마법 노딜수정) By 도우너 **/
	...
	private void runSkill() {
		...
		try {
			...
			for (Iterator<TargetStatus> iter = _targetList.iterator(); iter.hasNext();) {
				...
				if (cha.getSkillEffectTimerSet().hasSkillEffect(_skillId) && _skillId != SHOCK_STUN && _skillId != EMPIER) {
				...
				case SOLID_CARRIAGE:// 15
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.setAddBowReduction(Config.솔리드캐리지수치);
					}
					break;
				case BOUNCE_ATTACK: {
					L1PcInstance pc = (L1PcInstance) cha;
					pc.addDmgup(Config.바운스어택수치);
				}
					break;
				case DETECTION: {
					if (cha instanceof L1NpcInstance) {
				...
				case EMPIER: {
					...
					if (diffLevel < -2) {// 0226겜블수정
						// int[] stunTimeArray = { 700, 800, 900, 1000, 1100,
						// 1200, 1500, 1700, 2000 };
						int[] stunTimeArray = { 3000, 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if (diffLevel >= -2 && diffLevel <= 3) {
						// int[] stunTimeArray = { 800, 1000, 1400, 1500, 1600,
						// 1700, 2000, 2200, 2300, 2400, 2500, 3000 };
						int[] stunTimeArray = { 3100, 3200, 3300, 3400, 3500, 3600, 3700, 3800, 3900, 4000, 4100,
								4200 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if (diffLevel >= 4 && diffLevel <= 10) {
						// int[] stunTimeArray = { 1500, 1800, 2000, 2500, 2800,
						// 2900, 3000, 3500, 4000 };
						int[] stunTimeArray = { 3200, 3300, 3400, 3500, 3600, 3700, 3800, 3900, 4000 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if (diffLevel > 10) {
						// int[] stunTimeArray = { 2000, 2500, 3000, 3400, 3500,
						// 3600, 3700, 3800, 3900, 4000 };
						int[] stunTimeArray = { 3300, 3400, 3500, 3600, 3700, 3800, 3900, 4100, 4300, 4500 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					}

					L1EffectSpawn.getInstance().spawnEffect(4200012, _shockStunDuration, cha.getX(), cha.getY(),
							cha.getMapId());
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
				...
				case SHOCK_STUN: {
					...
					if (diffLevel < -2) {// 0226겜블수정
						// int[] stunTimeArray = { 700, 800, 900, 1000, 1100,
						// 1200, 1500, 1700, 2000 };
						int[] stunTimeArray = { 2000, 2000, 2100, 2100, 2100, 2200, 2200, 2200, 2300, 2400, 2500, 2600, 3000, 3100 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if (diffLevel >= -2 && diffLevel <= 3) {
						// int[] stunTimeArray = { 800, 1000, 1400, 1500, 1600,
						// 1700, 2000, 2200, 2300, 2400, 2500, 3000 };
						int[] stunTimeArray = { 2100, 2100, 2200, 2200, 2200, 2300, 2400, 2500, 2500, 3000, 3100, 3200, 3300, 3400 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if (diffLevel >= 4 && diffLevel <= 10) {
						// int[] stunTimeArray = { 1500, 1800, 2000, 2500, 2800,
						// 2900, 3000, 3500, 4000 };
						int[] stunTimeArray = { 2200, 2200, 2200, 2300, 2300, 2300, 2400, 2500, 2500, 2600, 3000, 3200, 3300, 3400 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					} else if (diffLevel > 10) {
						// int[] stunTimeArray = { 2000, 2500, 3000, 3400, 3500,
						// 3600, 3700, 3800, 3900, 4000 };
						int[] stunTimeArray = { 2500, 2500, 2500, 2700, 2700, 2700, 2900, 3100, 3300, 3500, 3700, 3900, 4000, 4100 };
						_shockStunDuration = stunTimeArray[random.nextInt(stunTimeArray.length)];
					}

					if (_skillId == 113) {
						cha.getSkillEffectTimerSet().setSkillEffect(113, _shockStunDuration);
						L1EffectSpawn.getInstance().spawnEffect(4200012, _shockStunDuration, cha.getX(), cha.getY(),
								cha.getMapId());
					} else {
				...
				case CANCELLATION: {
					if (cha instanceof L1PcInstance) {
						L1PcInstance pc = (L1PcInstance) cha;
						if (cha.getGfxId().getTempCharGfx() == 16074 || cha.getGfxId().getTempCharGfx() == 16053
								|| cha.getGfxId().getTempCharGfx() == 14491 || cha.getGfxId().getTempCharGfx() == 16056
								|| cha.getGfxId().getTempCharGfx() == 16284 || cha.getGfxId().getTempCharGfx() == 16002
								|| cha.getGfxId().getTempCharGfx() == 16040 || cha.getGfxId().getTempCharGfx() == 16027
								|| cha.getGfxId().getTempCharGfx() == 16014 || cha.getGfxId().getTempCharGfx() == 16008
								|| cha.getGfxId().getTempCharGfx() == 15986) {
							return;
						}
					}
					if (cha instanceof L1NpcInstance) {
				...
				case FOG_OF_SLEEPING: {
					if (cha instanceof L1PcInstance) {
						...
						L1CurseParalysis.sleeping(pc, 0, 32000, true);
					}
					cha.setSleeped(true);
				}
					break;
				...
				if (_calcType == PC_PC || _calcType == NPC_PC) { // 여기부터
					...
					case MASS_TELEPORT: {
						...
						if (bookm != null) {
							if (pc.getMap().isEscapable() || pc.isGm()) {
										...
										if (pc.getClanid() != 0 && member.getClanid() == pc.getClanid()
											...
											if (map.isInMap(newX2, newY2) && map.isPassable(newX2, newY2)) {
												L1Teleport.teleport(member, newX2, newY2, mapId,
														member.getMoveState().getHeading(), true);
											} else {
												pc.sendPackets(new S_AttackPacket(pc, 0, ActionCodes.ACTION_SkillBuff));
												Broadcaster.broadcastPacket(pc,
														new S_AttackPacket(pc, 0, ActionCodes.ACTION_SkillBuff));
								...
								if (pc.getInventory().checkItem(20288) || pc.getInventory().checkItem(120288)) {
									...
									if (map.isInMap(newX2, newY2) && map.isPassable(newX2, newY2)) {
										...
										L1Teleport.teleport(pc, newX, newY, mapId, pc.getMoveState().getHeading(),
												true);
									}
								}
							} else {
					...
					case SHADOW_FANG: {
						...
						if (item != null && item.getItem().getType2() == 1) {
							...
							pc.sendPackets(new S_SkillSound(pc.getId(), 8955)); // 아이콘
							pc._isShowFang = true;
						} else {
							pc.sendPackets(new S_ServerMessage(79));
						}
					...
					case BLOODY_SOUL: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.setCurrentMp(pc.getCurrentMp() + 20);// 12->20 수정
																// 본섭19
					}
						break;
					...
					case SHAPE_CHANGE: {
						...
						if (pc.getId() != target.getId()) {
							int probability = 200 - target.getResistance().getEffectedMrBySkill();
					...
					case STATUS_COMA_5: {
						...
						break;
					case STATUS_DRAGONMAAN_EARTH: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.MaanDodge = true;
						pc.getResistance().addPetrifaction(3);
					}
						break;
					case STATUS_DRAGONMAAN_WATER: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.MaanMagicIm = true;
						pc.getResistance().addFreeze(3);
					}
						break;
					case STATUS_DRAGONMAAN_WIND: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.MaanMagicDmg = true;
						pc.getResistance().addSleep(3);
					}
						break;
					case STATUS_DRAGONMAAN_FIRE: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.MaanAddDmg = true;
						pc.getResistance().addStun(3);
					}
						break;
					case STATUS_DRAGONMAAN_BIRTH: {
						L1PcInstance pc = (L1PcInstance) cha;
						pc.MaanDodge = true;
						pc.MaanMagicIm = true;
	...
	private boolean isNotCancelable(int skillNum) {
		return skillNum == ENCHANT_WEAPON || skillNum == BLESSED_ARMOR || skillNum == ABSOLUTE_BARRIER
				...
				|| skillNum == ARMOR_BRAKE || skillNum == REDUCTION_ARMOR || skillNum == SOLID_CARRIAGE
				|| skillNum == COUNTER_BARRIER || skillNum == UNCANNY_DODGE || skillNum == ARMOR_BRAKE;
	}
	...
	private boolean isUseCounterMagic(L1Character cha) {
		if (_isCounterMagic && cha.getSkillEffectTimerSet().hasSkillEffect(COUNTER_MAGIC)) {
			...
			Broadcaster.broadcastPacket(cha, new S_SkillSound(cha.getId(), 10702));
			if (cha instanceof L1PcInstance) {
				L1PcInstance pc = (L1PcInstance) cha;
