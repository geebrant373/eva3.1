			...
			try {
				if (action.equalsIgnoreCase("teleport mutant-dungen_la")) {
					for (L1PcInstance otherPc : L1World.getInstance().getVisiblePlayer(player, 3)) {
						if (otherPc.getClanid() != 0 && otherPc.getClanid() == player.getClanid()
					...
					TeleportDelyTimer timer = new TeleportDelyTimer();
					GeneralThreadPool.getInstance().execute(timer);
				} else if (action.equalsIgnoreCase("teleport phantasm-island")) { // 몽섬
					L1Teleport.teleport(player, 32633, 32817, (short) 303, 2, true);
				} else if (action.equalsIgnoreCase("teleport teberas")) { // 테베라스
					if (player.getWanted() == 0  && WantedTeleportTable.getInstance().isWantedTeleportMap(781)) {
						player.sendPackets(new S_SystemMessage("\\fT수배 상태에서만 사냥터로 이동 가능합니다."));
						return;
					}
					if (player.getLevel() >= Config.테베렙제) {
						...
						player.sendPackets(new S_SystemMessage(Config.테베렙제 + "렙부터 테베라스 사냥이 가능합니다."));
					}
				} else if (action.equalsIgnoreCase("teleport lastabad")) { // 라스타바드
					L1Teleport.teleport(player, 32729, 32851, (short) 453, 2, true);
				} else if (action.equalsIgnoreCase("teleport Island")) { // 잊혀진섬
					if (ForgottenIsleController.getInstance().isgameStart == true) {
						L1Teleport.teleport(player, 32832, 32849, (short) 70, 2, true);
					} else {
						player.sendPackets(new S_SystemMessage("\\fY잊혀진 섬이 열려있지 않습니다."));
					}
				} else if (action.equalsIgnoreCase("teleport sangatop")) { // 상아탑
					if (IvoryTowerController.getInstance().isgameStart == true) {
						L1Teleport.teleport(player, 32698, 32823, (short) 82, 2, true);
					} else {
						player.sendPackets(new S_SystemMessage("\\fY상아탑8층이 열려있지 않습니다."));
					}
				} else if (action.equalsIgnoreCase("teleport gludio")) { // 본던
					if (GludioDungeonController.getInstance().isgameStart == true) {
						L1Teleport.teleport(player, 32726, 32729, (short) 13, 2, true);
					} else {
						player.sendPackets(new S_SystemMessage("\\fY본던7층이 열려있지 않습니다."));
					}
				} else if (action.equalsIgnoreCase("teleport omantop")) { // 오만의탑정상층
					if (OmanTopFloorController.getInstance().isgameStart == true) {
						L1Teleport.teleport(player, 32734, 32864, (short) 200, 2, true);
					} else {
						player.sendPackets(new S_SystemMessage("\\fY오만의탑정상층이 열려있지 않습니다."));
					}
				}
			} catch (Exception e) {
			}
