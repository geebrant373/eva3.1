...
import l1j.server.server.model.Instance.L1NpcInstance.MprTimer;
...
public class L1NpcInstance extends L1Character {
	...
	protected L1NpcInstance _masterMannequinName = null;
	protected boolean _masterMannequin;
	...
	protected void setting_template(L1Npc template) {
		...
		if (this instanceof L1MannequinInstance || this instanceof L1ShopNpcInstance)
			setTitle(template.get_title());
	...
	public void onTarget() {
		...
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.DARKNESS) || getSkillEffectTimerSet().hasSkillEffect(L1SkillId.CURSE_BLIND))
			escapeDistance = 1;
		if (_target == null) {
			return;
		}
		...
		if (getAtkspeed() == 0 && getPassispeed() > 0) {
			...
			if (CharPosUtil.isAttackPosition(this, target.getX(), target.getY(), getNpcTemplate().get_ranged())) {// 기본
																													// 공격범위
	...
	public void setMasterMannequin(boolean i) {
		_masterMannequin = i;
	}

	public boolean getMasterMannequin() {
		return _masterMannequin;
	}

	public void setMasterMannequinName(L1NpcInstance npc) {
		_masterMannequinName = npc;
	}

	public L1Character getMasterMannequinName() {
		return _masterMannequinName;
	}
	...
	public void appearOnGround(L1PcInstance pc) {
		if (getHiddenStatus() == HIDDEN_STATUS_SINK) {
			...
			setActionStatus(0);
			Broadcaster.broadcastPacket(this, new S_NPCPack(this));
			if (!pc.getSkillEffectTimerSet().hasSkillEffect(60) && !pc.getSkillEffectTimerSet().hasSkillEffect(97) && !pc.isGm()) {
				_hateList.add(pc, 0);
				_target = pc;
			}
			onNpcAI();
	...
	public void setDirectionMove(int dir) {
		if (dir >= 0) {
			...
			if (getMovementDistance() > 0) {
				if (this instanceof L1GuardInstance || this instanceof L1CastleGuardInstance || this instanceof L1MerchantInstance
	...
	public void setalram(boolean _alram) {
		this._alram = _alram;
	}
	...
	public boolean isBoss() {
		if (getNpcId() == 45601 || getNpcId() == 46213 || getNpcId() == 45614 || getNpcId() == 45600 || getNpcId() == 45685 || getNpcId() == 45573
				|| getNpcId() == 45649 || getNpcId() == 45625 || getNpcId() == 45610 || getNpcId() == 45645) {
			return true;
		}
		return false;
	}
