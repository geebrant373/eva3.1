		...
		for (L1ArmorSets armorSets : ArmorSetTable.getInstance().getAllList()) {
			try {
				...
				impl.addEffect(new DamageEffect(armorSets.getSp(),
						...
						armorSets.getLongHitup(), armorSets.getLongDmgup(), armorSets.getReduction()));
...
class DamageEffect implements L1ArmorSetEffect {
	...
	public DamageEffect(int sp, int shortHitup, int shortDmgup, int longHitup, int longDmgup, int reduction){
		_sp = sp;
	...
	public void giveEffect(L1PcInstance pc) {
		...
		pc.addDamageReductionByArmor(_reduction);
