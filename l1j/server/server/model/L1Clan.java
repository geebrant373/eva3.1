...
public class L1Clan {
	static public class ClanMember {
		...
		public ClanMember(String name, int rank, boolean online, L1PcInstance pc) {
			this.name = name;
			this.rank = rank;
			this.online = online;
			this.player = pc;
	...
	public void addClanMember(String name, int rank, int online, L1PcInstance pc) {
		clanMemberList.add(new ClanMember(name, rank, online == 1, online == 1 ? pc : null));
	}
	...
	public String getOnlineMembersFP() {
		...
		int count = 0;
		if ("중립".equals(getClanName())) {
			String[] fakeMembers = { "운명", "환장하것소", "사사삿", "표정", "고곡", "승지니", "테스트트", "송지효", "천만십", "신념"
					, "코인사요", "본", "금메달", "다큰엘프", "에프킬러", "엠", "러브", "찍턴러", "막시무스", "사람", 
					"환술이", "못된오빠", "로나", "율이용기사", "다이", "흑월","슈가", "쿤타킨테", "양", "강토"
					, "긍", "기안", "학학", "눈", "천둥벼락", "가", "냥펀치", "쿠", "나이키", "링"};
