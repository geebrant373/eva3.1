...
public class L1CurseParalysis extends L1Paralysis {
	...
	private class ParalysisDelayTimer extends Thread {
		...
		public void run() {
			...
			if (!_target.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.FOG_OF_SLEEPING)) {
				((L1PcInstance) _target).sendPackets(new S_SkillIconAura(227, 8)); //추가 커스 패럴라이즈 아이콘 By.BENTLEY
			}
	...
	private L1CurseParalysis(L1Character cha, int delay, int time, boolean s) {
		_target = cha;
		_delay = delay;
		_time = time;
		sleeping();
	...
	public static boolean sleeping(L1Character cha, int delay, int time, boolean s) {
		if (!(cha instanceof L1PcInstance || cha instanceof L1MonsterInstance)) {
			return false;
		}
		cha.setParalaysis(new L1CurseParalysis(cha, delay, time, s));
		return true;
	}
	...
	public void cure() {
		...
		((L1PcInstance) _target).sendPackets(new S_SkillIconAura(227, 0.1)); //추가 커스 패럴라이즈 아이콘 By.BENTLEY
	}
}
