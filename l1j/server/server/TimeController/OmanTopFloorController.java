...
public class OmanTopFloorController implements Runnable {
    ...
    public static OmanTopFloorController getInstance() {
        if (_instance == null) {
            _instance = new OmanTopFloorController();
        }
        return _instance;
    }
        ...
        try {
            while (true) {
                ...
                case 대기:
                    ...
                    Status = 오픈;
                    L1World.getInstance().broadcastServerMessage("\\aH알림: 잠시후 [오만의탑 정상층] 입장이 가능합니다.");
                    continue;
                case 오픈:
                    L1World.getInstance().broadcastServerMessage("\\aH알림: 오만의탑 정상층이 2시간 사냥가능하오니,조심하세요.");
                    L1World.getInstance().broadcastServerMessage("\\aH알림: 시간이 되면 강제귀환 됩니다.");
                    System.out.println("...... 오만의탑 정상층 열림");
                    Status = 진행;
                    continue;
                case 진행:
                    Thread.sleep(3 * 1000);
                    for (L1PcInstance pc : L1World.getInstance().getAllPlayers()) {
                    	pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\aI5분뒤 오만의탑 정상층 보스 그림리퍼가 출현합니다."));
                    	pc.sendPackets(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, "\\aI5분뒤 오만의탑 정상층 보스 그림리퍼가 출현합니다."));
                    }
                    ...
                    continue;
                case 종료:
                    L1World.getInstance().broadcastServerMessage("\\aH알림: 오만의탑 정상층 종료되었습니다.");
                    System.out.println("...... 오만의탑 정상층 종료됨");
                    isgameStart = false;
                    Status = 대기;
                    continue;
                }
    ...
    private void TelePort() {
        for (L1PcInstance c : L1World.getInstance().getAllPlayers()) {
            ...
            case 190:
                ...
                c.sendPackets(new S_SystemMessage("오만의탑 정상층 닫혔습니다."));
                break;
            default:
                break;
            }
