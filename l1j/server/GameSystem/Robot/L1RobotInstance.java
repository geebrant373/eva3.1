...
public class L1RobotInstance extends L1PcInstance {
	public static boolean StartWar = false;
	...
	private static final int[] 리스봇BuffSkill4 = { L1SkillId.PHYSICAL_ENCHANT_STR, L1SkillId.PHYSICAL_ENCHANT_DEX,
	...
	class PotionThread implements Runnable {
		...
		public void run() {
			try {
				...
				if (percent < 5 && 사냥봇_타입 == HUNT && !사냥봇_위치.startsWith("잊섬") && !사냥봇_위치.startsWith("잊섬투")) {
					setCurrentHp(getCurrentHp() + 500);
					귀환();
					GeneralThreadPool.getInstance().schedule(this, 2000);
					return;
				} else if (percent < 15 && 사냥봇_타입 == HUNT && !사냥봇_위치.startsWith("잊섬") && !사냥봇_위치.startsWith("잊섬투")) {
					setCurrentHp(getCurrentHp() + 500);
					랜덤텔();
					GeneralThreadPool.getInstance().schedule(this, 2000);
					return;
	...
	private int Debuff() {
		// TODO 자동 생성된 메소드 스텁
		// 커스 대기
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.STATUS_CURSE_PARALYZING)) {
			귀환();
			딜레이 = 8000;
			return 8000;
		}
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.DECAY_POTION)) {
			귀환();
			int time = getSkillEffectTimerSet().getSkillEffectTimeSec(L1SkillId.DECAY_POTION) * 1000;
			return (int) (딜레이 = time);
		}
		if (getSkillEffectTimerSet().hasSkillEffect(L1SkillId.SILENCE)) {
			귀환();
			int time = getSkillEffectTimerSet().getSkillEffectTimeSec(L1SkillId.SILENCE) * 1000;
			return (int) (딜레이 = time);
		}
		return 0;
	}
		...
		try {
			...
			case TEL_NPC_MOVE:// 텔녀이동
				...
				if (isDistance(getX(), getY(), getMapId(), loc.getX(), loc.getY(), loc.getMapId(),
					...
					if (loc != null && 타_마을_텔이동) {
						...
						텔(loc.getX(), loc.getY(), loc.getMapId(), 3000 + _random.nextInt(3000));
						loc = location_queue.poll();
						타_마을_텔이동 = false;
					}
					if (loc == null) {
			...
			case DEATH: // 죽음
				...
				for (L1PcInstance pc2 : L1World.getInstance().getVisiblePlayer(this)) {
					pc2.sendPackets(new S_OtherCharPacks(this));
				}
				_target = null; // 누수방지
	...
	private void 추가SETTING좌표(Robot_Location_bean ro) {
		...
		if (ro.getX() == 33457 && ro.getY() == 32819 && ro.getMapId() == 4) {// 기란
																				// 물약상점
			if (getX() >= 34047 && getX() <= 34064 && getY() >= 32273 && getY() <= 32297 && getMapId() == 4) {// 오렌
				...
				location_queue.offer(new Robot_Location_bean(33438, 32796, 4));// 텔
																				// 할
																				// 위치
			} else if (getX() >= 33065 && getX() <= 33093 && getY() >= 33385 && getY() <= 33411 && getMapId() == 4) {// 은기사
			...
			if (location_queue.size() > 0)
				타_마을_텔이동 = true;
		} else if (ro.getX() == 33432 && ro.getY() == 32815 && ro.getMapId() == 4) {// 기란2 물약상점
			if (getX() >= 34047 && getX() <= 34064 && getY() >= 32273 && getY() <= 32297 && getMapId() == 4) {// 오렌
				location_queue.offer(new Robot_Location_bean(34064, 32278, 4));// 텔녀
																				// 위치
				location_queue.offer(new Robot_Location_bean(33438, 32796, 4));// 텔
			...
			if (location_queue.size() > 0)
				타_마을_텔이동 = true;
		} else if (ro.getX() == 33428 && ro.getY() == 32806 && ro.getMapId() == 4) {// 기란3,5 아덴상단
			if (getX() >= 34047 && getX() <= 34064 && getY() >= 32273 && getY() <= 32297 && getMapId() == 4) {// 오렌
				location_queue.offer(new Robot_Location_bean(34064, 32278, 4));// 텔녀
																				// 위치
				location_queue.offer(new Robot_Location_bean(33438, 32796, 4));// 텔
			...
			if (location_queue.size() > 0)
				타_마을_텔이동 = true;
		} else if (ro.getX() == 33437 && ro.getY() == 32803 && ro.getMapId() == 4) {// 기란4 젠도르
			if (getX() >= 34047 && getX() <= 34064 && getY() >= 32273 && getY() <= 32297 && getMapId() == 4) {// 오렌
				location_queue.offer(new Robot_Location_bean(34064, 32278, 4));// 텔녀
																				// 위치
				location_queue.offer(new Robot_Location_bean(33438, 32796, 4));// 텔
			...
			if (location_queue.size() > 0)
				타_마을_텔이동 = true;
		} else if (ro.getX() == 34065 && ro.getY() == 32287 && ro.getMapId() == 4) {// 오렌 물약상점
			if (getX() >= 33065 && getX() <= 33093 && getY() >= 33385 && getY() <= 33411 && getMapId() == 4) {// 은기사
				location_queue.offer(new Robot_Location_bean(33080, 33384, 4));
				location_queue.offer(new Robot_Location_bean(34062, 32278, 4));
			} else if (getX() >= 33410 && getX() <= 33461 && getY() >= 32788 && getY() <= 32838 && getMapId() == 4) {// 기란
				location_queue.offer(new Robot_Location_bean(33437, 32794, 4));
				location_queue.offer(new Robot_Location_bean(34062, 32278, 4));
			}
			if (location_queue.size() > 0)
				타_마을_텔이동 = true;
		} else if (ro.getX() == 32596 && ro.getY() == 32741 && ro.getMapId() == 4) {// 글말 물약상점
			if (getX() >= 33065 && getX() <= 33093 && getY() >= 33385 && getY() <= 33411 && getMapId() == 4) {// 은기사
				location_queue.offer(new Robot_Location_bean(33080, 33384, 4));
				...
				location_queue.offer(new Robot_Location_bean(33437, 32794, 4));
				location_queue.offer(new Robot_Location_bean(32608, 32734, 4));
			} else if (getX() >= 34047 && getX() <= 34064 && getY() >= 32273 && getY() <= 32297 && getMapId() == 4) {// 오렌
				location_queue.offer(new Robot_Location_bean(34064, 32278, 4));// 텔녀
																				// 위치
				location_queue.offer(new Robot_Location_bean(32608, 32734, 4));
			}
			if (location_queue.size() > 0)
				타_마을_텔이동 = true;
		} else if (ro.getX() == 33738 && ro.getY() == 32494 && ro.getMapId() == 4) {// 웰던 물약상점
			if (getX() >= 33065 && getX() <= 33093 && getY() >= 33385 && getY() <= 33411 && getMapId() == 4) {// 은기사
				location_queue.offer(new Robot_Location_bean(33080, 33384, 4));
	...
	private int Potion() {
		...
		if (getSkillEffectTimerSet().hasSkillEffect(POLLUTE_WATER) || getSkillEffectTimerSet().hasSkillEffect(10517)) { // 포르트워타중은
																														// 회복량1/2배
			healHp /= 2;
		}
	...
	private boolean checkTarget() {
		...
		if (_target != null) { // 때릴놈없을때
			return onTarget();// 이거 왜이럼???
	...
	public void onTargetItem() {
		...
		if (getLocation().getTileLineDistance(_targetItem.getLocation()) <= 1) {
			...
			int dir = moveDirection(_targetItem.getX(), _targetItem.getY(), _targetItem.getMapId());
			if (dir == -1) {
				_targetItem = null;
			} else {
	...
	public <L1Pcinstance> boolean onTarget2() {
		...
		if (!FirstSkill && !isSkillDelay() && getCurrentMp() > 70) {
			...
			if (skillId > 0) {
				if (CharPosUtil.isAttackPosition(this, target2.getX(), target2.getY(), target2.getMapId(),
						skill_range) == true
					...
					if (isKnight() && skillId == L1SkillId.SHOCK_STUN
							&& !target2.getSkillEffectTimerSet().hasSkillEffect(L1SkillId.SHOCK_STUN)) {
						...
						if (STrnd >= 7) {
							...
							par = null;
							L1SkillDelay.onSkillUse(this, 2000);
	...
	public void attackTarget(L1Character target) {
		...
		System.out.println("dddd="+attack.calcHit());
		if (attack.calcHit()) {
	...
	private void 이동(int x, int y) {
		...
		if (dir == -1) {
			...
			boolean tail2 = World.isThroughObject(getX(), getY(), getMapId(), dir);
			boolean door = World.문이동(getX(), getY(), getMapId(), calcheading(this, x, y));
			if (door || !tail2) {
				cnt++;
				if (cnt > 20) {
					딜레이(3000 + _random.nextInt(2000));
	...
	public void 텔(final int x, final int y, final int mapid, int time, final boolean effect) {
		if (사냥봇)
			item_queue.clear();
		GeneralThreadPool.getInstance().schedule(new Runnable() {
			@Override
	...
	private void 낚시봇() {
		...
		if (낚시좌표 == null) {
			L1Map map = L1WorldMap.getInstance().getMap((short) 5302);
			int wi = 50;
			while (wi-- > 0) {
				boolean ck = false;
	...
	public int calcSleepTime(int type) {
		...
		case MOVE:
			if (_move_gfxid != gfxid || _move_weapon != weapon) {
				_move_gfxid = gfxid;
				_move_weapon = weapon;
				_move_interval = SprTable.getInstance().getMoveSpeed(gfxid, weapon);
			}
