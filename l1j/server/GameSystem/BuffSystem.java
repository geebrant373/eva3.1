...
public class BuffSystem implements Runnable {
	...
	public void run() {
			try {
				...
				new L1SkillUse().handleCommands(_pc, L1SkillId.PHYSICAL_ENCHANT_DEX, _pc.getId(), _pc.getX(), _pc.getY(), null, 0 ,L1SkillUse.TYPE_GMBUFF);
				Broadcaster.broadcastPacket(npc1, new S_DoActionGFX(_npc.getId(), 19));
				new L1SkillUse().handleCommands(_pc, L1SkillId.PHYSICAL_ENCHANT_STR, _pc.getId(), _pc.getX(), _pc.getY(), null, 0 ,L1SkillUse.TYPE_GMBUFF);
				Broadcaster.broadcastPacket(npc1, new S_DoActionGFX(_npc.getId(), 19));
