	...
	protected void doSpawn(int spawnNumber, int objectId) {
		...
		try {
			...
			if (/* mob.getNpcId() == 45955 || */ mob.getNpcId() == 45956 || mob.getNpcId() == 45957
				...
				L1World.getInstance().broadcastPacketToAll(new S_SystemMessage("\\fY보스등장 : <\\fY"+mob.getName()+"\\fY>이(가) 출현하였습니다."));
				L1World.getInstance().broadcastPacketToAll(new S_PacketBox(S_PacketBox.GREEN_MESSAGE, " 보스알림: <" + mob.getName() +"> 이(가) 스폰 되었습니다."));
				L1World.getInstance().broadcastPacketToAll(new S_SystemMessage(" 보스알림: <" + mob.getName() +"> 이(가) 스폰 되었습니다."));
			}
		} catch (Exception e) {
