...
public class BattleZone implements Runnable {
	...
	private final ArrayList<L1PcInstance> line1 = new ArrayList<>();
	private final ArrayList<L1PcInstance> line2 = new ArrayList<>();
	
	public void addLine1(L1PcInstance pc) {
	    line1.add(pc);
	}

	public void addLine2(L1PcInstance pc) {
	    line2.add(pc);
	}
			...
			if (getDuelStart() == false) {
				...
				if (nowdueltime >= enddueltime || Close == true) {
					...
					if (count1 > count2) {
						...
						L1World.getInstance().broadcastServerMessage("\\fT* 배틀존 종료! '블루' 라인의 승리입니다 *");
					} else if (count1 < count2) {
						// 2번라인 우승
						winLine = 2;
						ment = "프리미엄 배틀존 '레드' 라인의 승리입니다.";
						L1World.getInstance().broadcastServerMessage("\\fT* 배틀존 종료! '레드' 라인의 승리입니다 *");
					} else {
						winLine = 3;
						ment = "프리미엄 배틀존 '블루' 라인과 '레드' 라인이 비겼습니다.";
						L1World.getInstance().broadcastServerMessage("\\fT* 배틀존 종료! '블루' 라인과 '레드'라인이 동점입니다 *");
					}
						...
						if (c2[i].get_DuelLine() != 0) {
							...
							if (c2[i].get_DuelLine() == winLine) {
								...
								for (int j = 0; j < itemIds.length; j++) {
									...
									if (item != null)
										c2[i].sendPackets(
												new S_SystemMessage(item.getName() + " (" + count + ")을 얻었습니다."));
								}
								c2[i].sendPackets(new S_SystemMessage("\\fT 승리팀에게 아이템이 지급되었습니다 *"));
							}
					...
					ment = null;
					Announcements.getInstance().announceToAll("\\fT프리미엄 배틀존이 종료되었습니다 *");
					set배틀존종료(true);
					set배틀존진행(false);
					setDuelStart(false);
					// DuelCount = 0;
	...
	private void 배틀존변신(L1PcInstance pc) {
		...
		if (pc != null) {
			if (pc.isKnight() || pc.isCrown() || pc.isDarkelf()) {
				// 기사 군주 다크엘프 용기사
				if (DuelLine == 1) {
					polyid = 6270;// <<1번라인 변신다크>
				} else {
					polyid = 6267;// 2번라인 아크변신
				}
	...
	public void 입장3분대기() {
		try {
			...
			if (getLine1Count() > 0 && getLine2Count() > 0) {
				balanceLineIfNeeded();
			    Announcements.getInstance().announceToAll("팀 인원이 충족되어 프리미엄 배틀존을 시작합니다.");
			    set배틀존진행(true);
			} else {
			    Announcements.getInstance().announceToAll("배틀존 인원이 충족되지 않아 이벤트가 취소되었습니다.");
			    for (L1PcInstance pc : toArray배틀존유저()) {
			        if (pc == null) continue;
			        pc.set_DuelLine(0);
	...
	public void balanceLineIfNeeded() {
	    int diff = Math.abs(line1.size() - line2.size());
	    ...
	    outPc.sendPackets(new S_SystemMessage(
	        "팀 인원 불균형으로 인해 배틀존에서 제외되었습니다."
	    ));

	    if (!outPc.isDead()) {
	        L1Teleport.teleport(outPc, 33090, 33402, (short) 4, 0, true);
	    }
	}
