	...
	public  void auth(LineageClient client, String accountName, String password, String ip, String host) throws IOException {
		...
		if (account == null) {			
			if (Config.AUTO_CREATE_ACCOUNTS) {
				if(Account.checkLoginIP(ip)) {
					client.sendPacket(new S_Notice("계정 생성이 초과되었습니다."));
					try {
						Thread.sleep(1500);
						client.kick();
						client.close();
		...
		
		{
			...
			for (L1PcInstance bugpc : pcs) {
				if (bugpc.getAccountName().equalsIgnoreCase(accountName)
						&& ((bugpc.isPrivateShop() == false && bugpc.isAutoClanjoin() == false) || bugpc.getNetConnection() != null)) {
					bugpc.getMap().setPassable(bugpc.getLocation(), true);
					bugpc.setX(33080);
					bugpc.setY(33392);
					bugpc.setMap((short) 4);
					System.out.println("─────────────────────────────────");
					System.out.println("버그로 판단되어 종료 합니다. Authorization - "+ accountName +"");
					System.out.println("─────────────────────────────────");
